<div x-data="{
    ...
    <template x-teleport="body">
        ...
        <div x-show="selectedProject" style="display: none;"
            ...
            <div @click="selectedProject = null" class="absolute inset-0 bg-black/90 backdrop-blur-md"></div>

            <div class="relative w-full max-w-5xl bg-rail-card rounded-[30px] shadow-2xl border border-white/10 overflow-hidden flex flex-col lg:flex-row max-h-[85vh] lg:h-[550px]"
                ...
                <div class="w-full lg:w-[50%] h-56 lg:h-full bg-gray-900 relative group shrink-0">
                    <img :src="selectedProject?.image"
                        class="w-full h-full object-cover opacity-90 transition-opacity hover:opacity-100">
                    <div
                        class="absolute inset-0 bg-gradient-to-t from-rail-card via-transparent to-transparent lg:bg-gradient-to-r lg:from-transparent lg:to-rail-card">
                    </div>
                </div>

                <div class="w-full lg:w-[50%] flex flex-col bg-rail-card">

                    <div class="p-6 lg:p-8 overflow-y-auto custom-scrollbar flex-1">
                        <span x-text="selectedProject?.category"
                            class="inline-block px-3 py-1 rounded-full bg-rail-dark text-rail-sweet text-xs font-bold uppercase tracking-wider mb-4 border border-white/10">
                        </span>

                        <h2 x-text="selectedProject?.title"
                            class="text-2xl md:text-3xl font-bold text-white mb-3 leading-tight"></h2>
                        <p x-text="selectedProject?.description"
                            class="text-gray-400 leading-relaxed text-sm md:text-base mb-6"></p>

                        <div class="mb-4">
                            <h4 class="text-xs font-bold text-gray-300 uppercase mb-3 flex items-center gap-2">
                                <svg class="w-4 h-4 text-rail-accent" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <template x-for="tech in selectedProject?.tech_stack">
                                    <span x-text="tech"
                                        class="px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-rail-accent text-xs font-medium hover:bg-white/10 transition-colors">
                                    </span>
                                </template>
                            </div>
                        </div>
                    ...
                    <div class="p-6 border-t border-white/5 bg-rail-card/50 backdrop-blur-sm mt-auto">
                        <div class="flex flex-col sm:flex-row gap-3">
                            <template x-if="selectedProject?.link_demo">
                                <a :href="selectedProject?.link_demo" target="_blank"
                                    class="flex-1 py-2.5 px-4 rounded-xl bg-gradient-to-r from-rail-accent to-rail-sweet text-white font-bold text-sm text-center shadow-lg hover:shadow-purple-500/30 transition-all hover:-translate-y-1 flex items-center justify-center gap-2">
                            ...
                            <template x-if="selectedProject?.link_github">
                                <a :href="selectedProject?.link_github" target="_blank"
                                    class="flex-1 py-2.5 px-4 rounded-xl bg-white/5 border border-white/10 text-gray-300 font-bold text-sm text-center hover:bg-white/10 hover:text-white transition-all flex items-center justify-center gap-2">
                            ...
                            <template x-if="selectedProject?.link_figma">
                                <a :href="selectedProject?.link_figma" target="_blank"
                                    class="flex-1 py-2.5 px-4 rounded-xl bg-white/5 border border-white/10 text-gray-300 font-bold text-sm text-center hover:bg-white/10 hover:text-pink-400 transition-all flex items-center justify-center gap-2">

                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                                        <path
                                            d="M12 2C13.6569 2 15 3.34315 15 5C15 6.65685 13.6569 8 12 8H15C16.6569 8 18 9.34315 18 11C18 12.6569 16.6569 14 15 14C13.3431 14 12 12.6569 12 11V2ZM8.5 2H12V8H8.5C6.84315 8 5.5 6.65685 5.5 5C5.5 3.34315 6.84315 2 8.5 2ZM5.5 11C5.5 9.34315 6.84315 8 8.5 8H12V14H8.5C6.84315 14 5.5 12.6569 5.5 11ZM12 14V17.5C12 19.433 10.433 21 8.5 21C6.567 21 5 19.433 5 17.5C5 15.567 6.567 14 8.5 14H12Z" />
                                    </svg>
